...
export default function SingleSelect<T>({ name, options, callback, label, value, optionDisplayKey, optionValueKey, fullwidth}: SingleSelectProps<T>) {
  ...
  return (
    <FormControl fullWidth={fullwidth} sx={{ margin: 1 }}>
      <InputLabel size="small" id={label+"-select-label"}>{label}</InputLabel>
        <Select
          variant="outlined"
          size="small"
          name={name}
