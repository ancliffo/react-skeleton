...
import type { SectionComponentSetProps } from "../../types/customTypes";
...
function SectionComponentSet<T>({
  ...
  const [componentSetInfo, setComponentSetInfo] =
    useState<UI_Section_ComponentSetSerializer | null>(null);
  const [componentSetMembers, setComponentSetMembers] = useState<
    UI_ComponentSet_MemberSerializer[] | null
  >(null);
    ...
    (newLayout: any) => {
      ...
      if (JSON.stringify(layout) !== JSON.stringify(newLayout)) {
        if (componentSetInfo && componentSetInfo.Layout_id) {
          apiUtils.updateAPIInfo(`${apiPath}/api/layout/${componentSetInfo.Layout_id}/`, {
            Layout: newLayout,
          });
        }
        setLayout(newLayout);
  ...
  async function getInitialMembers() {
    try {
      const tempComponentSetInfo = await apiUtils.getAPIInfo(
        `${apiPath}/api/section/component/set/${data.id}/`,
      );
      setComponentSetInfo(tempComponentSetInfo);
      const tempMembers = await apiUtils.getAPIInfo(
        `${apiPath}/api/component/set/member/${data.ComponentSet_id}/`,
      );
      setComponentSetMembers(tempMembers);
      let tempLayout =
        tempComponentSetInfo && tempComponentSetInfo.Layout_id
          ? await apiUtils.getAPIInfo(`${apiPath}/api/layout/${tempComponentSetInfo.Layout_id}/`)
          : null;
      if (tempMembers) {
        if (!tempLayout) {
          tempLayout = tempMembers
            ? tempMembers.map((item: any, index: number) => {
          ...
          if (dbLayoutObject) {
            ...
            apiUtils.updateAPIInfo(
              `${apiPath}/api/section/component/set/${data.id}/`,
              tempComponentSetInfo,
            );
