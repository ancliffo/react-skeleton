...
import type { SectionProps, SectionItem } from "../../types/customTypes";
...
function Section<T>({
  ...
  const [componentBehaviorMappings, setComponentBehaviorMappings] = useState<
    UI_ComponentBehavior_MappingSerializer[] | null
  >(null);
  ...
  const [sectionComponentSets, setSectionComponentsets] = useState<
    UI_Section_ComponentSetSerializer[]
  >([]);
    ...
    (newLayout: any) => {
      ...
      if (JSON.stringify(layout) !== JSON.stringify(newLayout)) {
        if (sectionInfo && sectionInfo.Layout_id) {
          apiUtils.updateAPIInfo(`${apiPath}/api/layout/${sectionInfo.Layout_id}/`, {
            Layout: newLayout,
          });
        }
        setLayout(newLayout);
  ...
  const getComponentSets = async () => {
    try {
      const tempComponentSets = await apiUtils.getAPIInfo(`${apiPath}/api/component/set/`);
  ...
  const getComponentBehaviorMappings = async () => {
    try {
      const tempBehaviorMappings = await apiUtils.getAPIInfo(
        `${apiPath}/api/component/behavior/mapping/`,
      );
  ...
  const getSectionAppearance = async () => {
    try {
      const tempAppearance = await apiUtils.getAPIInfo(
        `${apiPath}/api/section/appearance/${data.Appearance_id}/`,
      );
  ...
  async function getInitialSectionItems() {
    try {
      const tempSectionInfo = await apiUtils.getAPIInfo(`${apiPath}/api/section/${data.id}/`);
      ...
      let tempLayout =
        tempSectionInfo && tempSectionInfo.Layout_id
          ? await apiUtils.getAPIInfo(`${apiPath}/api/layout/${tempSectionInfo.Layout_id}/`)
          : null;
      if (tempSectionComponents.length > 0 || tempSectionComponentSets.length > 0) {
        const mergedItems = [...tempSectionComponents, ...tempSectionComponentSets];
        // or use custom merge logic
        setSectionItems(mergedItems);
        if (!tempLayout) {
          tempLayout = mergedItems
            ? mergedItems.map((item: SectionItem, index: number) => {
        ...
        //if we reached here, there are NO section items yet,
        // and there is NO layout stored - so store an empty layout
        // to match the stateful var initial value ([]):
        const dbLayoutObject = await apiUtils.createAPIInfo(`${apiPath}/api/layout/`, {
          Layout: [],
        });
        if (dbLayoutObject) {
  ...
  async function handleNewComponentSubmit(event: any) {
    ...
    formJson["Component_Behavior_Mapping_id"] = formJson["Component_Behavior_Mapping_id"].id
      ? formJson["Component_Behavior_Mapping_id"].id
      : formJson["Component_Behavior_Mapping_id"];
    ...
    const tempSectionCompopnents = await apiUtils.getAPIInfo(
      `${apiPath}/api/section/component/${data.id}/`,
    );
  ...
  async function handleNewComponentSetSubmit(event: any) {
    ...
    const tempSectionComponentSets = await apiUtils.getAPIInfo(
      `${apiPath}/api/section/component/set/list/${data.id}/`,
    );
    setSectionComponentsets(tempSectionComponentSets);
    // handleClose();
  }
              ...
              options={
                componentSets
                  ? componentSets.filter(
                      (set) =>
                        sectionComponentSets.some(
                          (sectionSet) => sectionSet.ComponentSet_id == set.id,
                        ) == false,
                    )
                  : []
              }
  ...
  {
    /* openButtonCallback--for keeping track of which thing was selected, 
    and what to display as form elements */
  }
  const hoverPopover = (handleClickOpen: (event: React.MouseEvent<HTMLButtonElement>) => void) => {
    return (
      <AddItemPopover callback={null}>
  ...
  return (
    <div style={{ height: "100%" }}>
      ...
      {sectionAppearance && sectionAppearance.Border && sectionAppearance.Panel ? (
        ...
        >
          <div
            style={{ backgroundColor: "rgb(204,229,255)", color: "rgb(55,100,160)", width: "100%" }}
          >
            <Stack sx={{ width: "100%" }} direction="row" spacing={1}>
              <div id={"accordian-summary" + data.id} key={"accordian-summary" + data.id}>
                <Typography component="span">{data.DisplayText}</Typography>
              </div>
          ...
          <div
            style={{
              border: sectionAppearance.BorderWidth + "px solid",
              borderRadius: 3,
              overflow: "auto",
              height: "90%",
            }}
